...
console.log(arr1);//2 "不大于3" 5 "大于3" 8 "大于3"   [2, 5, 8]
...
var returnValue = arr2.every(function (a) {//判断数组元素是否都是偶数，若有不满足的将不再进行后续判断
    console.log(a);//打开此行，查看是否会输出8，为什么？  不会 应为5不是偶数return false，循环就结束了
    return a%2===0;
});
...
three[0]=parseInt(a/100);
three[1]=parseInt(a%100/10);
three[2]=a%10;
var x=0;
for(i in three){
    x=x+three[i];
...
var returnValue = arr2.some(function (a) {//判断数组元素是否都是偶数，若有不满足的将不再进行后续判断
    //console.log(a);//打开此行，查看输出了哪些数，为什么？  2 为偶数，则不进行后续判断
    return a%2===0;
});
...
console.log(newArray,arr2);//[1,3,25,49,81]  [1,3,5,7,9]
...
var arr4 = ["a","b","c","d"];
console.log(arr4.reduce(printArgs));//prev a ,cur: b ,i: 1 prev ab ,cur: c ,i: 2 prev abc ,cur: d ,i: 3
 //abcd
console.log(arr4.reduce(printArgs,"x"));//prev x ,cur: a ,i: 0
//prev xa ,cur: b ,i: 1
//prev xab ,cur: c ,i: 2
//prev xabc ,cur: d ,i: 3
//xabcd
