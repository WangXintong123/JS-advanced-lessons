...
//加了g之后会改变lastIndex，如果匹配成功则改成成功的index，如果匹配不成功，则改为0
console.log(regExp.test("ab"));//true
console.log(regExp.test("ab"));//false 为什么？从a往后匹配，不成功
console.log(regExp.test("ab"));//true
...
console.log(retExp instanceof Array,retExp,execExp.lastIndex);// true "342", "4", "2", index: 3, input: "12s342dsfsf233s", groups: undefined] 0
console.log(retExp instanceof Array,retExp,execExp.lastIndex);// true "342", "4", "2", index: 3, input: "12s342dsfsf233s", groups: undefined] 0
...
while (ret2 = execExp2.exec(ts)){
    console.log(execExp2.lastIndex);//每次执行后从哪开始重新匹配？  6  14
}
*/