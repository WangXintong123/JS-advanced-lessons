...
try
{
    ...
    //console.log("x") = 23;//赋值引用错误，在chorme中测试   ReferenceError Invalid left-hand side in assignment
}catch(e){
...
try{
    var a;a.aa();
    //var a= new 123; //在chrome中测试   TypeError 123 is not a constructor
}catch(e){
    console.log(e.name,e.message);//TypeError Cannot read property 'aa' of undefined
}
finally {
    console.log("finally");//有无异常该句都会执行
}
...
try {
    ...
    console.log(e.message, e.name); // 传递异常对象到错误处理    InvalidMonthNo UserException
}
